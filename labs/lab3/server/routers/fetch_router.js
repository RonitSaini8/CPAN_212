...
router.get("/multiple", (req, res) => {
  // we read the files directory items synchronously to not trip the async speed
  let files_array = fs.readdirSync(upload_directory);
  // error checking
  if (files_array.length == 0) {
    // adding return will stop the rest of the operations
  ...
  // change these for multiple
  let filenames = _.sampleSize(files_array, 3);

  // 1 - send the file names
  // 2 - from the FE, request each file separately
  res.json(filenames);
});
